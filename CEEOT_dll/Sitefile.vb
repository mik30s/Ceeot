...
Public Module Sitefiles
    ...
    Public Function SiteFiles(ByRef index As String) As Object
        ...
        Do While X.AtEndOfStream <> True
            ...
            If Variables.Tag = "2" Then
                If (CDbl(Variables.Tag) = 2 And l > 0) Or (Initial.subareafile = 2 And l > 0) Then
                    numSub = numSub + 1
        ...
        F.Dispose()
        F = Nothing
        If Initial.subareafile = 2 And CDbl(Variables.Tag) = 2 Then
        ...
        Try
            ...
            With subarea
                For n = 0 To subarea.Rows.Count - 1
                    ...
                    Select Case Variables.ReachLenght
                        Case 0, 1
                ...
                If (.Rows(i).Item("SwatFile") <> "") Then
                    Select Case .Rows(i).Item("SwatFile")
                        Case "*.hru"
                            ...
                            Next
                        Case "bsn"
                            TakeField.filename = Initial.Input_Files & Initial.BaseFile
                            TakeField.Leng = .Rows(i).Item("Leng")
                            TakeField.LineNum = .Rows(i).Item("Lines")
                            TakeField.Inicia = .Rows(i).Item("Inicia")
            ...
            For z = 1 To UBound(hrufiles)
                ...
                If (hruname <> "") Then
                    ...
                    Else
                        ...
                        m = m + 1
                        linex(k) = hrufiles(z)
                        modifyLocalRecords("INSERT INTO APEXHRUs (Project,Subarea,HRU,Apex_Swat,pcpGage,Managment,Soil,Chemical,Groundwater,SoilName,LandUse) " &
                                         "VALUES('" & Initial.Project & "','" & filename & "','" & Left(linex(k), 13) & "','SWAT'," &
                                         Val(Mid(temp1, 49, 4)) & ",'" & Trim(Mid(linex(k), 14, 13)) & "','" &
                                         Trim(Mid(linex(k), 27, 13)) & "','" & Trim(Mid(linex(k), 40, 13)) & "','" &
                    ...
                    If flag1 = 1 Then
                        ...
                        Included = Nothing
                        modifyLocalRecords("INSERT INTO APEXHRUs (Project,Subarea,HRU,Apex_Swat,pcpGage,Managment,Soil,Chemical,Groundwater,SoilName,LandUse) " &
                                         "VALUES('" & Initial.Project & "','" & filename & "','" & Left(line1(l), 13) & "','" & apex_swat & "'," &
                                         Val(Mid(temp1, 49, 4)) & ",'" & Trim(Mid(line1(l), 14, 13)) & "','" &
                                         Trim(Mid(line1(l), 27, 13)) & "','" & Trim(Mid(line1(l), 40, 13)) & "','" &
